...
export class Handler {
	...
	static fromRoute = createHandlerFromRoute;
	...
	async handle(req: IncomingMessage, res: ServerResponse, urlParams: Record<string, string | undefined>) {
		...
		try {
			...
			if (err instanceof Error) {
				console.error('Handler generated an error!',err);
...
export function createHandlerFromRoute(
	...
	// 5. Add interceptors
	handler.rawInterceptors = [
		...handler.rawInterceptors,
		...route.rawInterceptor ?? []
	];
	handler.requestInterceptors = [
		...handler.requestInterceptors,
	...
	handler.responseInterceptors = [
		...handler.responseInterceptors,
		...route.responseInterceptor ?? []
	];
